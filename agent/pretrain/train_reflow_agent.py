...
class TrainReFlowAgent(PreTrainAgent):
        ...
        env_config = EnvConfig(
            ...
            wrappers={
                "mujoco_locomotion_lowdim": {
                    "normalization_path": f"/home/zhangtonghe/0-dppo-video-mujoco/data/gym/{cfg.env}/normalization.npz"
                },
                "multi_step": {
    ...
    def test(self):
        ...
        for step in tqdm(range(self.n_steps)) if VERBOSE_LOG else range(self.n_steps):
            ...
            with torch.no_grad():
                ...
                self.model: ReFlow
                # print(f"cond['state'].shape={cond['state'].shape}")s
                samples = self.model.sample(cond, inference_steps=self.model.max_denoising_steps, inference_batch_size=self.env_config.n_envs, record_intermediate=False)
    ...
    def run(self):
        timer = Timer()
        self.epoch = 1
        for epoch in tqdm(range(self.n_epochs)) if VERBOSE_LOG else range(self.n_epochs):
            ...
            for step, batch_train in enumerate(self.dataloader_train):
                ...
                t: Type = <class 'torch.Tensor'>, Shape = torch.Size([128])
                obs: Type = <class 'torch.Tensor'>, Shape = torch.Size([128, 1, 11])
                v: Type = <class 'torch.Tensor'>, Shape = torch.Size([128, 4, 3])
                print_type_and_shape("act", act)
                print_type_and_shape("cond", cond)
                print_type_and_shape("xt", xt)
                print_type_and_shape("t", t)
                print_type_and_shape("obs", obs)
                print_type_and_shape("v", v)
                '''
                
                self.optimizer.zero_grad()
                
                loss_train = self.model.loss(xt, t, cond, v)
            ...
            # save model # default is 100 by pre_ReFlow_mlp.yaml
            ...
            if self.epoch % self.log_freq == 0:  # default is every step. 
                # test
                avg_traj_length, success_rate, avg_episode_reward, avg_episode_reward_std, avg_best_reward, avg_best_reward_std=self.test()
                    ...
                    wandb.log(
                        {
                            ...
                            "avg_episode_reward_std": avg_episode_reward_std,
                            "avg_best_reward": avg_best_reward,
                            "avg_best_reward_std": avg_best_reward_std,

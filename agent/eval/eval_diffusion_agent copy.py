...
class EvalDiffusionAgent(EvalAgent):
    ...
    def run(self):
        ...
        time = timer()
        log.info(
                f"eval: num episode {num_episode_finished:4d} | success rate {success_rate:8.4f} | avg episode reward {avg_episode_reward:8.1f}±{avg_episode_reward_std:2.1f} | avg best reward {avg_best_reward:8.1f}±{avg_best_reward_std:2.1f} |"
            )

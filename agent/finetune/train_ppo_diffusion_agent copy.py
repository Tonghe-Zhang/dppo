        ...
        while self.itr < self.n_train_itr:
            ...
            for step in tqdm(
                ...
                with torch.no_grad():
                    ...
                    # print(f"chains_venv.shape={chains_venv.shape}")# chains_venv.shape=(40, 11, 4, 3), here 11 is 10+1, 10 is the finetune denoising steps
            ...
            if not eval_mode:
                with torch.no_grad():
                    ...
                    returns_trajs = advantages_trajs + values_trajs     # Q functions
                ...
                print(f"chains_trajs.shape={chains_trajs.shape}")  #(500,40,11,4,3)
                chains_k = einops.rearrange(
                    torch.tensor(chains_trajs, device=self.device).float(),
                    "s e t h d -> (s e) t h d",
                )
                print(f"chains_k.shape={chains_k.shape}") #(20000,11,4,3)
                returns_k = (
                ...
                print(f"self.n_steps={self.n_steps} * self.n_envs={self.n_envs} * self.model.ft_denoising_steps={self.model.ft_denoising_steps}")
                clipfracs = []
                
                for update_epoch in tqdm(range(self.update_epochs),desc="\t\tUpdate Actor/Critic"):
                    ...
                    for batch in range(num_batch):
                        ...
                        print(f"chains_k.shape={chains_k.shape}")
                        chains_prev_b = chains_k[batch_inds_b, denoising_inds_b]   ##(20000,11,4,3)
                        chains_next_b = chains_k[batch_inds_b, denoising_inds_b + 1]
                        
                        print(f"chains_prev_b.shape={chains_prev_b.shape}") #(50,000,4,3)
                        print(f"chains_next_b.shape={chains_next_b.shape}")#(50,000,4,3)
                        """ 
                        self.n_steps=500 * self.n_envs=40 * self.model.ft_denoising_steps=10
                        self.batch_size=50000:                                                                                                                                                    | 0/5 [0
                        num_batch=4
                        chains_k.shape=torch.Size([20000, 11, 4, 3])
                        chains_prev_b.shape=torch.Size([50000, 4, 3])
                        chains_next_b.shape=torch.Size([50000, 4, 3])
                        """

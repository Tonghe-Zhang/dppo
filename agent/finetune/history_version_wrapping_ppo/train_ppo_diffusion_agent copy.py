...
from agent.finetune.buffer import PPODiffusionBuffer
...
class TrainPPODiffusionAgent(TrainPPOAgent):
    ...
    def get_value(self, cond:dict, device='cpu'):
        ...
        if device == 'cpu':
            ...
            value_venv = self.model.critic.forward(cond).float().to(self.device)
        return value_venv
    ...
    def run(self):
        self.prepare_run(device=self.device) #GPU version

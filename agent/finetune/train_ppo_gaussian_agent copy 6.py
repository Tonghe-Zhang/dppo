...
success upon copy 4, minor adjustment. 
"""
import os
import pickle
import einops
...
class TrainPPOGaussianAgent(TrainPPOAgent):
    ...
    def get_values(self):
        ...
        for t, obs_venv in enumerate(self.obs_trajs["state"]):             # obs_venv: [self.n_envs, self.n_cond_step, self.obs_dim]
            ...
            self.value_trajs[t] = self.model.critic(obs_venv).cpu().numpy().flatten()
    ...
    def get_logprobs(self): 
        ...
        for t, (obs_venv, actions_venv) in enumerate(zip(obs_t, samples_t)):
            self.logprobs_trajs[t]  = self.model.get_logprobs({"state": obs_venv}, actions_venv)[0].cpu().numpy()
    ...
    def run(self):
        ...
        while self.itr < self.n_train_itr:
            ...
            for step in range(self.n_steps):
                ...
                self.save_full_observation(info_venv)
    ...
    def log(self):
        ...
        if self.itr % self.log_freq == 0:
            ...
            if self.eval_mode:
                log.info(
                    f"Evaluation at self.itr={self.itr}: success rate {self.success_rate:8.3f} | avg episode reward {self.avg_episode_reward:8.3f} | avg best reward (per action) {self.avg_best_reward:8.3f}"
                )
                if self.use_wandb:
                    wandb.log(
                        {

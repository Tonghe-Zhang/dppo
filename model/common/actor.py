...
class ActorObs(torch.nn.Module):
    """State-only Actor network."""
    def __init__(
        self,
        cond_dim,   # observation history, To  x Do
        action_dim, #
        horizon_steps, # action horizon, action chunk
        mlp_dims,
        ...
        **kwargs,
        ...
        super().__init__()
        mlp_dims = [cond_dim] + mlp_dims + [action_dim*horizon_steps]
        if residual_style:
            model = ResidualMLP
        else:
            model = MLP
    ...
    def forward(self, cond: Union[dict, torch.Tensor]):
        ...
        return action_mean.view(-1, self.Ta, self.Da)


class ActorObsAct(torch.nn.Module):
    """State-action double Actor network."""

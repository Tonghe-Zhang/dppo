...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"emb[None, :].shape={emb[None, :].shape}")
        # print(f"x.shape={x.shape}")
        # print(f"x[:, None].shape={x[:, None].shape}")
        emb = x[:, None] * emb[None, :]
        # print(f"x[:, None] * emb[None, :].shape={emb.shape}")
        emb = torch.cat((emb.sin(), emb.cos()), dim=-1)
        return emb
        # half_dim=8
        # emb.shape=torch.Size([8])
        # emb[None, :].shape=torch.Size([1, 8])
        # x.shape=torch.Size([32, 1])
        # x[:, None].shape=torch.Size([32, 1, 1])
        # x[:, None] * emb[None, :].shape=torch.Size([32, 1, 8])

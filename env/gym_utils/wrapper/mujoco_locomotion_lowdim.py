...
class MujocoLocomotionLowdimWrapper(gym.Env):
    ...
    def step(self, action):
        # Tonghe debug: print(f"Call: step(self, action): in /root/autodl-tmp/dppo/env/gym_utils/wrapper/mujoco_locomotion_lowdim.py")
        raw_action = self.unnormalize_action(action)
        import inspect
        # Tonghe debug: print(f"Will call self.env.step(raw_action) in {inspect.getfile(self.env.step)}, with source ={inspect.getsource(self.env.step)}")
        raw_obs, reward, done, info = self.env.step(raw_action)

        # normalize
        obs = self.normalize_obs(raw_obs)
        # Tonghe debug: print(f"Call: complete, return four objects in /root/autodl-tmp/dppo/env/gym_utils/wrapper/mujoco_locomotion_lowdim.py")

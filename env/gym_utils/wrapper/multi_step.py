...
class MultiStep(gym.Wrapper):
    ...
    def step(self, action):
        ...
        """
        # Tonghe debug: print(f"Call: def step(self, action): in /root/autodl-tmp/dppo/env/gym_utils/wrapper/multi_step.py")
        ...
        # Tonghe debug: print(f"action={action}")
        for act_step, act in enumerate(action):
            self.cnt += 1
            if terminated or truncated:
                break
            # done does not differentiate terminal and truncation
            # Tonghe debug: print(f"Call: self.env.step(act) in /root/autodl-tmp/dppo/env/gym_utils/wrapper/multi_step.py")
            # Tonghe debug: import inspect 
            # Tonghe debug: print(f"Will call self.env.step(act) in {inspect.getfile(self.env.step)}, with source ={inspect.getsource(self.env.step)}")
            observation, reward, done, info = self.env.step(act)
            # Tonghe debug: print(f"Call-completed, received four objects. self.env.step(act) in /root/autodl-tmp/dppo/env/gym_utils/wrapper/multi_step.py")
        ...
        # Tonghe debug: print(f"self._get_obs")

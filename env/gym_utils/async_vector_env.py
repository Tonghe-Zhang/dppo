...
class AsyncVectorEnv(VectorEnv):
    ...
    def reset_async(
        ...
        for pipe, single_seed in zip(self.parent_pipes, seed):
            ...
            # print(f">>>pipe.send() single_kwargs={single_kwargs}")
    ...
    def call_async(self, name, *args, **kwargs):
        ...
        **kwargs
        ...
        for pipe in self.parent_pipes:
            # print(f"name=={name}, args={args}, kwargs={kwargs}")# name==render, args=(), kwargs={'mode': 'human'}
...
def _worker_shared_memory(index, env_fn, pipe, parent_pipe, shared_memory, error_queue):
    ...
    try:
        while True:
            command, data = pipe.recv()
            if command == "reset":
                # print(f">>>command==reset in shared memory")
                if "return_info" in data and data["return_info"] == True:
                    # print(f">>>command==reset in shared memory--branch1")
                    observation, info = env.reset(**data)
                    write_to_shared_memory(
                    ...
                    # print(f">>>command==reset in shared memory--branch2")
                    # import inspect
                    # example_function =env.reset
                    # file_location = inspect.getfile(example_function)
                    # source_code = inspect.getsource(example_function)
                    # print(f"Function is defined in: {file_location}")
                    # print(f"Source code of the function:{source_code}")
                ...
                name, args, kwargs = data
                # print(f'''calling command == "_call", name, args, kwargs={name, args, kwargs}''')#calling command == "_call", name, args, kwargs=('render', (), {'mode': 'human'})
                if name in ["reset", "step", "seed", "close"]:
                    raise ValueError(
                        f"Trying to call function `{name}` with "
                        f"`_call`. Use `{name}` directly instead."

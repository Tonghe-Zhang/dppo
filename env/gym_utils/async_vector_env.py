...
class AsyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        print(f"Multiprocessing: target==_worker_shared_memory?{target==_worker_shared_memory} or _worker?{target==_worker}")
        target = worker or target
        print(f"Multiprocessing: target==worker?{target==worker}")
        """ 
        target is _worker_shared_memory
        """
    ...
    def step_async(self, actions):
        ...
        AlreadyPendingCallError
            ...
            between.
        """
        #Tonghe debug: print(f"Call: def step_async(self, actions): in /root/autodl-tmp/dppo/env/gym_utils/async_vector_env.py")
        self._assert_is_running()
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(
        ...
        for pipe, action in zip(self.parent_pipes, actions):
            #Tonghe debug: print(f"Call: pipe.send(('step',action)) in def step_async(self, actions) of /root/autodl-tmp/dppo/env/gym_utils/async_vector_env.py")
            pipe.send(("step", action))
        self._state = AsyncState.WAITING_STEP

    def step_wait(self, timeout=None):
        #Tonghe debug: print(f"Call: def step_wait(self, timeout=None): in /root/autodl-tmp/dppo/env/gym_utils/async_vector_env.py")
        ...
        #Tonghe debug: print(f"Call: def step_wait(self, timeout=None):, Returns: results={results}, successes={successes} in def step_wait(self, timeout=None):")
        self._raise_if_errors(successes)
        self._state = AsyncState.DEFAULT
    ...
    def _raise_if_errors(self, successes):
        #Tonghe debug: print(f"Call: def _raise_if_errors(self, successes): in /root/autodl-tmp/dppo/env/gym_utils/async_vector_env.py")
        if all(successes):
            return
        ...
        for _ in range(num_errors):
            #Tonghe debug: print(f"\t\t num_errors={num_errors}")
    ...
    try:
        while True:
            ...
            if command == "reset":
                ...
                print(f"Call: worker called with command 'step'. ")
    ...
    try:
        while True:
            ...
            if command == "reset":
                ...
                # Tonghe debug: print(f"Call: _worker_shared_memory with command=='step'. ")
                # Tonghe debug: print(f"inspect.getsource(env.step)={inspect.getsource(env.step)}")
                # Tonghe debug: print(f"inspect.getfile(env.step)={inspect.getfile(env.step)}")
                
                result_env_step_data= env.step(data)
                
                observation, reward, terminated, truncated, info=result_env_step_data
                
                write_to_shared_memory(
                    observation_space, index, observation, shared_memory
                )
                #Tonghe debug: print(f"Call: _worker_shared_memory with command=='step', pipe.send")
